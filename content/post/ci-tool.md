...
You are probably just having a knee jerk reaction. You probably have a problem with your pipelines.
...
While that definition is true, I think it mislead people intro treating it as a magic black box. A sausage factory for production artefacts. As developers are not paid to create code, but rather to deploy complete and secure features to customers, that's counterproductive in my opinion.
...
It's not jenkins fault if agents' authentication to nexus is failing; it's not TeamCity fault if your agents don't have connectivity to the DEV environment anymore. You can't blame your CI if you didn't provide enough agents and builds are queued. It's not Bamboo fault if some other build left files behind and broke your build - the agent did run _exactly_ the commands it was told to do.

I can tell you this by personal experience: of all the components to maintain in CI-land, the CI tool is the one less likely to give us trouble. The easy part is maintaining that service running.
...
{{%expand "So why can't I run the jobs from my machine instead?" %}}
<br/>
...
Changing the CI tool was a big waste of money for the company in almost every case I've seen. Costs and effort are always underestimated, heavily. Your pipelines are a lot more complicated and interact with a lot more subsystems than people can realise. There's a surprising amount of code which relies on your CI tool specificities.

You have a _working_ CI tool, with all your proper pipelines in place, and you've already spent time customising. A new CI tool means more servers, more ops, more automation, more security assessments, the migration, more glue code, learning and adapting every single pipeline you know to the new workarounds, authentication, permissions, training, learning curve, those _all_ cost money. Anything you estimate, make it at least double.

The reason a company buys a CI is not out of generosity; it's buying a piece of software which will actually increase developers' and ops' productivity. The CI is not a business deliverable, it's a tool to improve quality and speed of the products being delivered.
...
Is it because CI is such a central tool? Is it because the management wouldn't know better? Because people want to leave a personal mark? It is because of all the other CI problems actually manifested themselves on the CI tool screen, kind of some collective exposure trauma? Is it because people simply don't understand that was broken, and blame the CI tool?

I've seen people migrating to (and from) GoCD because they wanted _every_ single artefact in the company to be part of the same pipeline/stream map. Just because GoCD allows you to do that, it doesn't mean you should - even the Continuous Delivery book tells you it's a bad idea. Also, it doesn't mean you are forced to do it just because you are in GoCD!

If your life is hell because you tangled all your dependencies together on the same pipeline, _fix your pipelines_! Don't blame GoCD for your bad pipelines.
...
Are you actually blaming Bamboo because you decided to not use its plan/pipelines the way the tool was designed to be, but rather jobs like Jenkins? Fix your pipelines.
...
What's the underlining problem you are trying to address when you want to change the CI tool? Ask that carefully, in ten different ways. Chances are the problem is a completely different thing, just being masked as CI tool hate.

I do believe that, while there are legit reasons to migrate CI tools, I've never seen them happening. Even Jenkins can serve as a good CI tool, and sometimes it had enough love and care to not be worth migrating.


It's not a matter of what's the best CI tool, but if there's real benefit on migrating.
